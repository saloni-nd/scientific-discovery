...
# Import
# Make sure filename is correct, or add by-sex.txt at the end.
...
# Check filename is correct or add External-by-sex.txt to the end of it
sub_df <- read_tsv(paste0(data_folder, "Underlying Cause of Death, 2018-2021, Single Race - External-by-sex.txt"))
# Make sure colnames are in the correct order
colnames(sub_df) <- c("Notes", "Age_long", "Age",  "ICD_subchapter_long", "ICD_subchapter", "Gender_long", "Gender", "Deaths_n", "Population", "Death_crude_rate")
...
coded_df1 <- coded_df %>%
              ...
              select(c(Notes, Age_long, Age, Gender_long, ICD_long, ICD, Deaths_n, Death_crude_rate, Population))

sub_df1 <- sub_df %>%
              select(c(Notes, Age_long, Age, Gender_long, ICD_subchapter_long, ICD_subchapter, Deaths_n, Death_crude_rate,Population))

colnames(sub_df1) <- c("Notes", "Age_long", "Age", "Gender_long", "ICD_long", "ICD", "Deaths_n", "Death_crude_rate", "Population")

joined_df <- bind_rows(coded_df1, sub_df1)

# Calculate the % of deaths in each age and gender group that are in each ICD code
joined_df <- joined_df %>%
  group_by(Age, Gender_long) %>%
  mutate(Total_Deaths_Group = sum(Deaths_n)) %>%
  ungroup() %>%
  mutate(Percentage_Deaths_ICD = (Deaths_n / Total_Deaths_Group) * 100)
...
ggplot(joined_df, aes(x = Age, y = Percentage_Deaths_ICD, fill = ICD_long)) +
  ...
  labs(
    title = "What are people dying from at different ages? [External causes highlighted]",
...
ggsave(paste0(data_folder, "cod_lifespan_external_share_bysex.svg"), width=8, height=10)
...
ggplot(joined_df, aes(x = Age, y = Deaths_n, fill = ICD_long)) +
  ...
  facet_wrap(~ Gender_long, scales = "free_y", nrow = 2) + 
  labs(
    title = "How many people are dying from each cause at different ages? [External causes highlighted]",
...
ggsave(paste0(data_folder, "cod_lifespan_external_number_bysex.svg"), width=8, height=10)
...
# Females selected - change to M to see chart for males
ggplot(filter(sub_df, Gender=="F"), aes(x = Age, y = Death_crude_rate, color = ICD_subchapter_long)) +
  ...
  labs(
    title = "How do the risks of death from each cause vary with age? [External causes only]",
...
ggplot(joined_df, aes(x = Age, y = Death_crude_rate, fill = ICD_long)) +
  #geom_bar(stat = "identity", position = "fill", alpha = 0.7) +
...
ggsave(paste0(data_folder, "cod_lifespan_externalrates_bysex.svg"), width=8, height=10)
